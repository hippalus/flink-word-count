...
public final class DataStreamJob {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final DataStream<String> text = newTextStream(params, env);

    final DataStream<Tuple2<String, Integer>> counts =
        // The text lines read from the source are split into words
        // using a user-defined function. The tokenizer, implemented below,
        // will output each word as a (2-tuple) containing (word, 1)
        text.flatMap(Tokenizer.createTokenizer())
            .name("tokenizer")
  ...
  private static DataStream<String> newTextStream(final CLI params, final StreamExecutionEnvironment env) {
    final DataStream<String> text;
    if (params.getInputs().isPresent()) {
      // Create a new file source that will read files from a given set of directories.
      // Each file will be processed as plain text and split based on newlines.
      final var builder = FileSource.forRecordStreamFormat(new TextLineInputFormat(), params.getInputs().get());
  ...
  /**
   * Implements the string tokenizer that splits sentences into words as a user-defined FlatMapFunction. The function takes a line
   * (String) and splits it into multiple pairs in the form of "(word,1)" ({@code Tuple2<String, Integer>}).
   */
  public static final class Tokenizer implements FlatMapFunction<String, Tuple2<String, Integer>> {
    ...
    public void flatMap(final String value, final Collector<Tuple2<String, Integer>> out) {
      ...
      final String[] tokens = PATTERN.split(value.toLowerCase());
